        ...
        <div className="relative mt-1">
          ...
          <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
            <Listbox.Options className="absolute z-40 mt-1 max-h-60 w-full overflow-auto rounded-xl bg-white py-1 shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
              {options.map((option) => (
                <Listbox.Option key={option.leaveTypeId} value={option} disabled={option.disabled} className={({ active, disabled }) => `relative cursor-default select-none py-2 pl-4 pr-4 ${ active && !disabled ? 'bg-indigo-100 text-indigo-900' : 'text-gray-900' } ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
                  {({ selected }) => (
                    <div className="flex justify-between items-center">
                       <span className={`block truncate ${selected ? 'font-semibold' : 'font-normal'}`}>{option.leaveName}</span>
                       <span className={`text-xs ${selected ? 'font-medium' : 'text-gray-500'}`}>{option.availableText}</span>
                       {selected && <span className="absolute inset-y-0 right-2 flex items-center text-indigo-600"><CheckIcon className="h-5 w-5" /></span>}
                    </div>
                  )}
                </Listbox.Option>
              ))}
...
export default function ManagerEditLeaveRequest({ isOpen, onClose, onSave, requestDetails }) {
  ...
  useEffect(() => {
    if (isOpen && requestDetails) {
      ...
      const isCustom = requestDetails.isHalfDay || (requestDetails.startSession && requestDetails.startSession !== 'none');
      setShowCustomHalfDay(isCustom);
      setHalfDayConfig({
        start: isCustom && requestDetails.startSession !== 'none' ? requestDetails.startSession : 'fullday',
        end: isCustom && requestDetails.endSession !== 'none' ? requestDetails.endSession : 'fullday',
      });
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            {loadingData ? (
              <div className="mt-1 w-full p-2.5 border rounded-md bg-gray-100 text-center text-gray-500 text-sm">Loading balances...</div>
            ) : (
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <button type="submit" disabled={submitting || loadingData} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50">
