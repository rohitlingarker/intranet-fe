...
function countWeekdaysBetween(fromDate, toDate, halfDayConfig) {
  ...
  let total = 0;
  // By appending 'T00:00:00Z', we force the date to be parsed in UTC.
  // This is the crucial fix that prevents local timezone issues.
  const current = new Date(fromDate + 'T00:00:00Z');
  const end = new Date(toDate + 'T00:00:00Z');

  // Loop through each day
  while (current <= end) {
    // getUTCDay() is used to get the day of the week in UTC (0=Sun, 6=Sat)
    ...
    if (dayOfWeek !== 0 && dayOfWeek !== 6) {
      ...
      // This core calculation logic remains the same, as it was correct.
      // The bug was in the date looping, not here.
      if (isStartDate) {
    ...
    // Safely advance to the next day in UTC
        ...
        <div className="relative mt-1">
          ...
          <Transition as={Fragment} leave="transition opacity-100 duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
            <Listbox.Options className="absolute z-40 mt-1 max-h-60 w-full overflow-auto rounded-xl bg-white py-1 shadow-xl ring-1 ring-black ring-opacity-5">
              {options.map((option) => (
                <Listbox.Option key={option.leaveTypeId} value={option} disabled={option.disabled} className={({ active, disabled }) => `py-3 px-4 flex items-center justify-between cursor-pointer select-none ${active && !disabled ? "bg-indigo-50 text-indigo-900" : ""} ${disabled ? "opacity-40 cursor-not-allowed" : ""}`}>
                  {({ selected }) => (
                    <>
                      <span className={`${selected ? "font-medium" : ""} ${ option.disabled ? "text-gray-400" : "" }`}>
                        {option.leaveName}
                      </span>
                      <span className={`ml-4 text-xs ${ option.disabled ? "text-gray-400" : "text-gray-500" }`}>
                        {option.availableText}
                      </span>
                      {selected && (<span className="absolute inset-y-0 right-2 flex items-center text-indigo-600"><CheckIcon className="h-4 w-4" /></span>)}
                    </>
                  )}
                </Listbox.Option>
              ))}
...
export default function RequestLeaveModal({ isOpen, onClose, onSuccess }) {
  ...
  const shouldShowDriveLink = () => {
    ...
    if (isSickLeave) { return enoughDays; }
    return requiredDocs;
  };
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => { setBalances(response.data); setLoadingBalances(false); })
      .catch(() => { setBalanceError("Failed to load leave balances."); setLoadingBalances(false); });
  }, [isOpen, employeeId]);
  ...
  const handleSubmit = async (e) => {
    ...
    const daysRequested = countWeekdaysBetween(startDate, endDate, halfDayConfig);
    ...
    try {
      ...
      onSuccess?.(); 
      setTimeout(() => {
        setStartDate(""); setEndDate(""); setLeaveTypeId(""); setReason("");
        setShowCustomHalfDay(false); setHalfDayConfig({ start: 'none', end: 'none' });
        setDriveLink(""); onClose();
      }, 1000);
    } catch (err) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-2 sm:mx-4 max-h-[90vh] overflow-y-auto border border-gray-100 relative">
        <div className="sticky top-0 bg-white z-10 p-3 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors rounded-full p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" type="button" aria-label="Close">
              <X className="w-6 h-6" />
            </button>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-6">
          {(error || success) && (
            <div className={`rounded-xl p-3 ${ error ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700" }`}>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <input type="date" value={endDate} onChange={handleEndDateChange} min={startDate} required className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
          
          <div className="text-center space-y-1">
            <span className="inline-flex items-center px-2 py-1 rounded-full text-lg font-semibold bg-indigo-100 text-indigo-700">
              {weekdays} day{weekdays !== 1 ? "s" : ""}
            </span>
            <p className="text-xs text-gray-500">Weekends are excluded from count.</p>
          </div>

          <div className="space-y-1">
            <label className="block text-sm font-medium text-gray-700">Leave Type <span className="text-red-500">*</span></label>
            {loadingBalances ? (
              <div className="flex items-center justify-center p-4 text-gray-500 text-sm rounded-xl bg-gray-50"><span className="animate-pulse mr-2">⏳</span> Loading...</div>
            ) : balanceError ? (
              <div className="text-red-600 text-sm bg-red-50 rounded-xl p-2">{balanceError}</div>
            ) : (
          ...
          <div className="space-y-1">
            ...
            <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Add a reason" rows={4} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 resize-none"/>
          </div>

          {shouldShowDriveLink() && (
            <div className="space-y-1">
              <label className="block text-sm font-medium text-gray-700">Supporting Document (Google Drive Link) <span className="text-red-500">*</span></label>
              <input type="text" value={driveLink} onChange={(e) => setDriveLink(e.target.value)} placeholder="https://drive.google.com/..." required={shouldShowDriveLink()} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"/>
              {selectedLeaveType?.leaveTypeId === "L-SL" && weekdays > 3 && (
                <p className="text-xs text-gray-500 mt-1">For sick leave exceeding 3 days, please attach supporting document.</p>
              )}
            </div>
          )}

          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-5 border-t border-gray-200">
            <button type="button" onClick={onClose} className="px-5 py-3 rounded-lg font-medium text-gray-800 border border-gray-300 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" disabled={submitting}>
              Cancel
            </button>
            <button type="submit" className="px-5 py-3 rounded-lg font-medium bg-indigo-600 text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-60 disabled:cursor-not-allowed" disabled={submitting || loadingBalances || !startDate || !endDate || !leaveTypeId}>
              {submitting ? (
                <span className="flex items-center justify-center"><span className="animate-spin mr-2">⟳</span> Requesting...</span>
              ) : ( "Request Leave" )}
            </button>
          </div>
        </form>
