...
const handleExportPDF = () => {
    ...
    // -------- TITLE --------
    pdf.setFontSize(18);
    pdf.text(`Financial Report — ${monthName} ${data.year}`, 14, 15);
    ...
    // -------- PROJECT MEMBERS BREAKDOWN (SEPARATE PAGES) --------
    data.projectUserHoursBreakdown.forEach((proj) => {
      pdf.addPage();
      pdf.setFontSize(14);
      pdf.text(`Project: ${proj.projectName}`, 14, 15);
    ...
    autoTable(pdf, {
      startY: pdf.lastAutoTable.finalY + 10,
      head: [["Employee", "No. of Days", "Hours", "Contribution"]],
    ...
    // -------- REPORT NOTES --------
    pdf.setFontSize(12);
    pdf.text("Report Notes", 14, pdf.lastAutoTable.finalY + 10);

    const notes = [
      "Billable Hours = Total hours spent on tasks classified as billable across all projects.",
      "Standard Holiday Hours = 8 hours per holiday (Mon–Fri).",
      "Non-Billable Hours = Non-billable tasks + Holiday hours.",
    ...
    let y = pdf.lastAutoTable.finalY + 18;
    pdf.setFontSize(10);
