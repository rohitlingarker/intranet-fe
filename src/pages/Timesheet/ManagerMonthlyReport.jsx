...
const generateManagerPDF = async (apiData) => {
  ...
  // Title
  doc.setFontSize(18);
  doc.text("Manager Monthly Report", 14, 14);

  doc.setFontSize(11);
  doc.text(
    `Reporting Period: ${apiData.dateRange.startDate} to ${apiData.dateRange.endDate}`,
    14,
    22
  );
  ...
  addTable(
    [["Metric", "Value"]],
    [
      ["Total Hours", `${apiData.totalHours}h`],
      ["Billable Hours", `${apiData.billableHours}h`],
      ["Non-Billable Hours", `${apiData.nonBillableHours}h`],
      ["Auto Generated Hours", `${apiData.autoGeneratedHours}h`],
      ["Billable %", `${apiData.billablePercentage}%`],
      ["Active Projects", apiData.uniqueProjectCount],
      ["Pending Users", apiData.pending || 0],
    ]
  );
  ...
  addTable(
    [["Day", "Hours"]],
    Object.entries(apiData.weeklySummary).map(([d, h]) => [d, h])
  );

  // PROJECT HOURS SUMMARY
  addTable(
    [["Project", "Billable", "Non-Billable", "Total", "%"]],
    apiData.projectHoursSummary.projectHours.map((p) => [
      p.projectName,
      p.billableHours,
      p.nonBillableHours,
      p.totalHours,
  ...
  // PROJECT BREAKDOWN PER PROJECT (CONTINUOUS)
  apiData.projectBreakdown.forEach((project) => {
    doc.setFontSize(13);
    ...
    doc.text(`Project: ${project.projectName}`, 14, currentY);
    currentY += 6;

    addTable(
      [["Employee", "Hours", "Billable", "Non-Billable", "Contribution %"]],
      project.membersContribution.map((m) => [
        m.userName,
        m.totalHours,
        m.billableHours,
        m.nonBillableHours,
  ...
  doc.text("Billable Contribution", 14, currentY);
  currentY += 6;

  addTable(
    [["Employee", "Billable Hours", "Contribution %"]],
    apiData.billableContribution.members.map((m) => [
      m.userName,
      m.billableHours,
      `${m.contribution}%`,
    ])
  ...
  // NON-BILLABLE CONTRIBUTION
  doc.setFontSize(13);
  if (currentY + 10 > 280) {
    doc.addPage();
    currentY = 20;
  }
  doc.text("Non-Billable Contribution", 14, currentY);
  currentY += 6;

  addTable(
    [["Employee", "Non-Billable Hours", "Contribution %"]],
    apiData.nonBillableContribution.members.map((m) => [
      m.userName,
      m.nonBillableHours,
      `${m.contribution}%`,
    ])
  ...
  doc.text("Auto Generated Hours", 14, currentY);
  currentY += 6;

  addTable(
    [["Employee", "Auto Hours", "Contribution %"]],
    apiData.autoGeneratedContribution.members.map((m) => [
      m.userName,
      m.autoHours,
      `${m.contribution}%`,
    ])
  ...
  addTable(
    [["User", "Pending Weeks"]],
    apiData.pendingUsers.map((u) => [u.userName, u.pendingWeeks])
  );

// EMPLOYEE TIMESHEET SUMMARY
  (apiData.userEntriesSummary || []).forEach((user) => {
    doc.setFontSize(13);
    if (currentY + 10 > 280) {
      doc.addPage();
      currentY = 20;
    ...
    // User header
    doc.setFontSize(12);
    doc.text(`${user.userName || user.name || "User"}`, 14, currentY);
    currentY += 6;
    ...
    weeks.forEach((week) => {
      ...
      doc.setFontSize(12);
      doc.text(
        `Week ${week.weekId} (${week.startDate} to ${week.endDate})`,
        14,
        currentY
      );
      currentY += 6;
      ...
      addTable(
        [["Date", "Hours", "Status"]],
        (week.timesheets || []).map((t) => [
          t.workDate || t.date || "-",
          t.hoursWorked ?? t.hours ?? "-",
          t.status || "-",
        ])
      );
...
// (Removed duplicate static box declarations — dynamic sizing block below handles Report Notes box)
...
const notes = [
  ...
  "Total Hours: Sum of Billable Hours + Non-Billable Hours.",
  ...
  "Active Projects: Count of all projects that had at least one time entry in the selected month.",
  "Project Allocation Hours: Total number of hours contributed to each project by all assigned team members.",
...
let boxX = 12;
let boxWidth = 186;
let textWidth = boxWidth - 20; // left padding (10), bullet space (10)
let startY = currentY;

// 1️⃣ CALCULATE TOTAL HEIGHT AFTER WRAPPING TEXT
let lineHeight = 6;
let totalHeight = 20; // title + top padding

const wrappedNotes = notes.map(note => {
  let wrapped = doc.splitTextToSize(note, textWidth);
  totalHeight += wrapped.length * lineHeight + 4; // extra spacing
  return wrapped;
});
...
// If the box exceeds page height → new page
if (startY + totalHeight > 280) {
  doc.addPage();
  startY = 20;
}

// 2️⃣ DRAW DYNAMIC BOX
doc.setFillColor(230, 240, 255);
doc.roundedRect(boxX, startY, boxWidth, totalHeight, 5, 5, "F");

// 3️⃣ DRAW TITLE
let cursorY = startY + 12;
doc.setFontSize(12);
doc.setTextColor(30, 60, 180);
doc.text("Report Notes", boxX + 8, cursorY);

cursorY += 10;

// 4️⃣ DRAW WRAPPED NOTES INSIDE BOX
doc.setFontSize(10);
doc.setTextColor(0, 0, 0);

wrappedNotes.forEach(wrappedLines => {
  // bullet
  doc.setFillColor(90, 90, 90);
  doc.circle(boxX + 6, cursorY + 2, 1.8, "F");

  // text
  doc.text(wrappedLines, boxX + 12, cursorY);
...
currentY = cursorY + 10;
  ...
  // SAVE PDF
  doc.save(`Manager_Report_${apiData.month}_${apiData.year}.pdf`);
};
