...
function TimelineHeader({ zoom, ticks, todayOffset, totalDays, dayWidth }) {
  return (
    <div className="relative h-10 border-b bg-muted/30 z-10" style={{ width: `${totalDays * dayWidth}px` }}>
      {ticks.map((tick, i) => (
        <div
          key={i}
...
function RoleAggregateRow({ role, resources, startDate, endDate, dayWidth, todayOffset, rowHeight, onResourceClick, ticks, weeklyTicks }) {
  const totalDays = daysBetween(startDate, endDate)
      ...
      (res.allocationTimeline || []).forEach(block => {
        ...
        if (bEnd >= startDate && bStart <= endDate) {
          ...
          if (!projects.has(key)) {
            ...
            if (parseDate(block.startDate) < parseDate(existing.startDate)) existing.startDate = block.startDate
            if (parseDate(block.endDate) > parseDate(existing.endDate)) existing.endDate = block.endDate
  ...
  useEffect(() => {
    if (scrollRef.current && currentDate) {
      ...
      const timer = setTimeout(() => {
        ...
        scrollRef.current.scrollLeft = targetOffset - scrollRef.current.clientWidth / 3
      }, 100)
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 border-b p-3 bg-white relative z-50">
        <div className="flex items-center justify-between sm:justify-start gap-4">
          <Tabs value={viewMode} onValueChange={(v) => setViewMode(v)}>
            <TabsList className="h-8 bg-muted/50 p-1">
              <TabsTrigger value="resource" className="text-[10px] h-6 gap-1.5 px-2 sm:px-3 rounded-sm">
                <User className="h-3 w-3" />
                <span className="hidden xs:inline">Resource</span>
                <span className="xs:hidden">Resources</span>
              </TabsTrigger>
              <TabsTrigger value="role" className="text-[10px] h-6 gap-1.5 px-2 sm:px-3 rounded-sm">
        ...
        <div className="flex flex-wrap items-center justify-between sm:justify-start gap-2 sm:gap-3">
          ...
          <Button
            variant="outline"
            size="sm"
            className="h-7 px-2 sm:px-3 text-[10px] font-semibold bg-white"
            onClick={scrollToToday}
          >
            Today
          </Button>

          <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-[10px] font-medium text-muted-foreground sm:border-l sm:pl-3">
                    ...
                    >
                      ...
                      <div className="min-w-0 text-left">
                        <div className="flex items-center gap-1 sm:gap-2 min-w-0">
                          <p className="text-[10px] sm:text-xs font-heading font-bold text-foreground truncate min-w-0 flex-1">{resource.name}</p>
                        ...
                        <p className="text-[8px] sm:text-[10px] text-muted-foreground truncate">{resource.role}</p>
                      </div>
                    </button>
                  )
                })
                ...
                [...roleGroups.entries()].map(([role, resources]) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ height: `${dynamicRowHeight}px` }}
                    >
                      <p className="text-sm font-heading font-bold text-slate-800 truncate">{role}</p>
