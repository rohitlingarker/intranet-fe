...
const SearchableCombobox = ({ label, value, onChange, options, error = false }) => {
  const [query, setQuery] = useState("");
  const filteredOptions = query === "" ? options : options.filter((o) => o.toLowerCase().replace(/\s+/g, "").includes(query.toLowerCase().replace(/\s+/g, "")));
  ...
  return (
    <div className="w-full">
      ...
      <Combobox value={value} onChange={onChange}>
        <div className="relative mt-1">
          <div className={`relative w-full cursor-default overflow-hidden rounded-md border bg-white text-left shadow-sm focus:outline-none sm:text-sm ${error ? "border-red-500 ring-1 ring-red-500" : "border-gray-300"}`}>
            <Combobox.Input
            ...
            <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-50">
              {filteredOptions.length === 0 && query !== "" ? (
                ...
                filteredOptions.map((option) => (
                  <Combobox.Option key={option} className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? "bg-blue-100 text-blue-900" : "text-gray-900"}`} value={option}>
                    {({ selected, active }) => (
                      <>
                        <span className={`block truncate ${selected ? "font-medium" : "font-normal"}`}>{option}</span>
                        {selected && (
...
const CreateClient = ({ mode, initialData, onSuccess }) => {
  ...
  const handleCountryChange = (selectedCountryName) => {
    ...
    const uniqueOffsets = [...new Set(tzIds.map((id) => `UTC${ct.getTimezone(id).utcOffsetStr}`))];
    ...
    setErrors(errors.filter(f => f !== "country_name" && f !== "default_timezone"));
  };
  ...
  const handleSubmit = async () => {
    ...
    const mandatoryFields = ["client_name", "client_type", "priority_level", "delivery_model", "status", "country_name", "default_timezone"];
