...
import { Search, MoreVertical, CheckCircle2, AlertTriangle } from "lucide-react";
...
const RMSProjectList = () => {
  ...
  if (loading) return <div className="p-10 text-center">Syncing with PMS...</div>;
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={loadData} className="bg-[#263383] text-white px-4 py-2 rounded text-sm font-medium hover:bg-[#1e2a6b]">
          Sync PMS Data
        </button>
      </div>
          ...
          >
            ...
            <div className="p-5 pb-3">
              <div className="flex justify-between items-start mb-2">
                <span className={`px-2 py-1 text-[10px] font-bold uppercase rounded border ${
                    project.riskLevel === 'HIGH' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-green-50 text-green-600 border-green-100'
                }`}>
            ...
            <div className="px-5 py-2">
              ...
              <div className="w-full bg-gray-100 rounded-full h-1.5">
                <div className="bg-[#263383] h-1.5 rounded-full" style={{ width: `${project.progress}%` }}></div>
              </div>
            </div>

            <div className="p-4 border-t border-gray-100 mt-2 flex justify-between items-center text-sm">
                <div className="flex items-center gap-2">
                    {project.readiness === 'Ready' ? <CheckCircle2 className="h-4 w-4 text-green-600" /> : <AlertTriangle className="h-4 w-4 text-amber-500" />}
                    <span className="text-gray-600 font-medium">{project.readiness}</span>
                </div>
                <div className="font-bold text-gray-700">{project.projectBudgetCurrency} {project.projectBudget.toLocaleString()}</div>
            </div>
          </div>
        ))}
      </div>
