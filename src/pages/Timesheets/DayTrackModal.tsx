...
const DayTrackModal: React.FC = () => {
  ...
  const [entries, setEntries] = useState<ProjectTaskEntry[]>([
    {
      projectId: 0,
      tasks: [{ taskName: "", description: "", startTime: "", endTime: "" }],
      workType: "",
    },
  ]);
  ...
  useEffect(() => {
    setProjects([
      ...
      { id: 3, name: "Project C", tasks: ["Task 5", "Task 6"] },
    ]);
  }, []);

  class RandomNumberGenerator {
    static generate(min: number = 1000, max: number = 10000): number {
  ...
  const handleChange = (
    ...
    if (field === "projectId") {
      ...
      updated[index].tasks = [
        { taskName: "", description: "", startTime: "", endTime: "" },
      ];
    } else {
  ...
  const handleSubmit = async () => {
    ...
    const payload = entries.flatMap((entry) =>
      entry.tasks.map((task) => ({
        ...
        taskId: RandomNumberGenerator.generate(), // 🔄 Assuming taskName holds task ID as string
        description: task.description,
        workType: entry.workType,
        fromTime: `${selectedDate}T${task.startTime}`,
        toTime: `${selectedDate}T${task.endTime}`,
        otherDescription: task.showOtherDescription? task.otherDescription : "Nothing",
      }))
    );

    try {
      const response = await fetch(
        `http://localhost:8080/api/timesheet?workDate=${selectedDate}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        alert("Failed to submit timesheet: " + error);
      }
    } catch (err) {
      console.error("Network or server error:", err);
      alert("Network error while submitting timesheet.");
    }
  };
  return (
    <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#263383] bg-opacity-40">
          <div className="bg-white p-6 rounded-lg w-full max-w-3xl shadow-lg relative overflow-y-auto max-h-[90vh] scrollbar-hide">
            <div className="flex justify-center items-center mb-6">
              <h2 className="text-2xl font-bold text-[#495ab6]">
                DAILY TIME SHEET⌛
              </h2>
            </div>
                ...
                >
                  <div className="flex items-end gap-2 mb-3">
                    ...
                    <button
                      ...
                      className="bg-[#263383] text-white px-3 py-1 rounded text-sm"
                    >
                      + Project
                    </button>
                  </div>
                  ...
                  {entry.tasks.map((taskEntry, taskIdx) => {
                    const usedTasks = entry.tasks
                      .map((t) => t.taskName)
                      .filter(Boolean);
                    const filteredTasks = availableTasks.filter(
                      (task) =>
                        !usedTasks.includes(task) || task === taskEntry.taskName
                    );

                    return (
                      <div key={taskIdx} className="mb-4 border rounded p-2">
                        <div className="flex items-start gap-2">
                          <div className="w-[50%]">
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                const updated = [...entries];
                                updated[index].tasks[taskIdx].taskName =
                                  e.target.value;
                                setEntries(updated);
                              }}
                            >
                          ...
                          {taskIdx === entry.tasks.length - 1 && (
                            <button
                              disabled={
                                !taskEntry.taskName ||
                                entry.tasks.length >= availableTasks.length
                              }
                              onClick={() => {
                                const updated = [...entries];
                              ...
                              className="bg-green-600 text-white px-3 py-1 mt-6 rounded"
                            >
                              + Task
                            </button>
                          )}
                        ...
                        <div className="mt-2">
                          ...
                          <textarea
                            ...
                            onChange={(e) => {
                              const updated = [...entries];
                              updated[index].tasks[taskIdx].description =
                                e.target.value;
                              setEntries(updated);
                            }}
                          />
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = [...entries];
                                updated[index].tasks[taskIdx].startTime =
                                  e.target.value;
                                setEntries(updated);
                              }}
                            />
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = [...entries];
                                updated[index].tasks[taskIdx].endTime =
                                  e.target.value;
                                setEntries(updated);
                              }}
                            />
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              updated[index].tasks[
                                taskIdx
                              ].showOtherDescription = e.target.checked;
                              setEntries(updated);
                            }}
                          />
                          <label
                            htmlFor={`others-${index}-${taskIdx}`}
                            className="text-sm"
                          >
                            Other
                          </label>
                        ...
                        {taskEntry.showOtherDescription && (
                          <div className="mt-2">
                            ...
                            <textarea
                              ...
                              onChange={(e) => {
                                const updated = [...entries];
                                updated[index].tasks[taskIdx].otherDescription =
                                  e.target.value;
                                setEntries(updated);
                              }}
                            />
...
const DayTrackModal: React.FC = () => {
   const [timesheetEntries, setTimesheetEntries] = useState<TimesheetEntryType[]>([]);
  const [showModal, setShowModal] = useState(false);
  const [selectedDate, setSelectedDate] = useState<string>(new Date().toISOString().split('T')[0]);
  const [projects, setProjects] = useState<Project[]>([]);
  const [entries, setEntries] = useState<ProjectTaskEntry[]>([{
  ...
  const handleChange = (index: number, field: keyof ProjectTaskEntry, value: any) => {
    ...
    if (field === 'projectId') {
      updated[index].projectId = value;
      updated[index].tasks = [{ taskName: '', description: '', startTime: '', endTime: '' }];
    } else {
      (updated[index] as any)[field] = value;
    }
    setEntries(updated);
  ...
  const handleSubmit = async () => {
    ...
    const payload = entries.flatMap((entry) =>
      entry.tasks.map((task) => ({
        ...
        description: task.description,
        workType: entry.workType,
        fromTime: task.startTime,
        toTime: task.endTime,
        otherDescription: task.showOtherDescription ? task.otherDescription || '' : '',
      }))
    );

    try {
      const response = await fetch(`/api/timesheet?workDate=${selectedDate}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#263383] bg-opacity-40">
          <div className="bg-white p-6 rounded-lg w-full max-w-3xl shadow-lg relative overflow-y-auto max-h-[90vh] scrollbar-hide">
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          {taskIdx === entry.tasks.length - 1 && (
                            <button
                              ...
                              onClick={() => {
                                const updated = [...entries];
                                updated[index].tasks.push({ taskName: '', description: '', startTime: '', endTime: '' });
                                setEntries(updated);
                              }}
                              className="bg-green-600 text-white px-3 py-1 mt-6 rounded"
                            >
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = [...entries];
                                updated[index].tasks[taskIdx].startTime = e.target.value;
                                setEntries(updated);
                              }}
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = [...entries];
                                updated[index].tasks[taskIdx].endTime = e.target.value;
                                setEntries(updated);
                              }}
                            />
                          </div>
            ...
            <div className="flex justify-end space-x-4 mt-4">
              <button onClick={handleSubmit} className="bg-[#263383] text-white px-4 py-2 rounded" >
                Submit
              </button>
