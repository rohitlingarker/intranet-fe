...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const projectRes = await axiosInstance.get(`/api/projects/member/${userId}`);
        setProjects(Array.isArray(projectRes.data) ? projectRes.data : []);

        const storyRes = await axiosInstance.get(`/api/stories/assignee/${userId}`);
        setStories(Array.isArray(storyRes.data) ? storyRes.data : []);

        const taskRes = await axiosInstance.get(`/api/tasks/assignee/${userId}`);
    ...
    <li key={project.id} className="bg-white rounded-xl shadow p-4">
      ...
      {expandedProjects === project.id && (
        <div className="mt-4 border-t pt-4 text-gray-700 text-sm space-y-2">
          ...
          {project.owner && <p><strong>Owner:</strong> {project.owner.name}</p>}
          {project.members && project.members.length > 0 && (
            <p><strong>Members:</strong> {project.members.map((m) => m.name).join(", ")}</p>
          )}
          {project.createdAt && <p><strong>Created At:</strong> {new Date(project.createdAt).toLocaleString()}</p>}
          {project.updatedAt && <p><strong>Updated At:</strong> {new Date(project.updatedAt).toLocaleString()}</p>}
    ...
    <li key={story.id} className="bg-white rounded-xl shadow p-4">
      ...
      {expandedStories === story.id && (
        <div className="mt-2 border-t pt-2 text-gray-700 text-sm space-y-1">
          ...
          {story.epic && <p><strong>Epic:</strong> {story.epic.title}</p>}
          {story.reporter && <p><strong>Reporter:</strong> {story.reporter.name}</p>}
          {story.assignee && <p><strong>Assignee:</strong> {story.assignee.name}</p>}
          ...
          {story.project && (
            <div className="pt-2">
              <Button
                variant="primary"
                size="small"
                onClick={() => navigate(`/projects/${story.project.id}`)}
              >
                Go to Project Tabs
              </Button>
            </div>
    ...
    <li key={task.id} className="bg-white rounded-xl shadow p-4">
      ...
      {expandedTasks === task.id && (
        <div className="mt-2 border-t pt-2 text-gray-700 text-sm space-y-1">
          ...
          {task.story && <p><strong>Story:</strong> {task.story.title}</p>}
          {task.assignee && <p><strong>Assignee:</strong> {task.assignee.name}</p>}
          {task.reporter && <p><strong>Reporter:</strong> {task.reporter.name}</p>}
          ...
          {task.story?.project && (
            <div className="pt-2">
              <Button
                variant="primary"
                size="small"
                onClick={() => navigate(`/projects/${task.story.project.id}`)}
              >
                Go to Project Tabs
              </Button>
            </div>
